...
| `/book` | `/tr/book` | `src/app/[locale]/book/page.tsx` | `page.hasBooking` | `SubpageHero` + ayrı bir `BookingCalendar` bileşeni **gerekmiyor** (revize edildi) — booking overlay zaten gerçek Cal.com embed'i kullanıyor (`@calcom/embed-react`, `BookingOverlay.tsx`), `/book` sayfası aynı `<Cal calLink=... />`'ı overlay chrome'u olmadan, dc.html'in kendi kart çerçevesi (`page.underCal` metni dahil) içinde doğrudan gömecek |
