 ...
 * (kelime kelime makine çevirisi değil).
 *
 * İKİNCİ TUR (kullanıcı geri bildirimi sonrası): sabit record/reading/
 * gap/call/note şablonu ve corrections/how-it-works aparatı TAMAMEN
 * KALDIRILDI. Episode 01 (Bó) artık serbest biçimli bir anlatı —
 * başlıklar, paragraflar, satır içi kaynak linkleri, stat highlight'lar
 * ve fspark9 notları. Hiçbir tarih, rakam, alıntı ya da kaynak icat
 * edilmedi — hepsi build prompt'un orijinal "content spine"ından.
 * Doğrulanmamış ön-tarih (Mart/Eylül 2018) olayları ve resmi "gap"
 * aparatı bilinçli olarak anlatıdan çıkarıldı — yeni yön daha kısa,
 * daha akıcı bir hikaye istiyor, ansiklopedik eksiksizlik değil.
 *
 * Çalıştırma:
...
const lastDayFormatDoc = {
  ...
  hero: {
    ...
    // Kullanıcı geri bildirimi: "sadece kapanan şirketleri bulacağını
    // anlayacak bir text görecek, bu kadar" — how-it-works ve
    // corrections paneli tamamen kaldırıldı.
    intro: lt(
      "Companies and products that shut down — read what happened.",
      "Kapanan şirketler ve ürünler — ne olduğunu okuyun.",
    ),
  },
...
// ─────────────────────────────────────────────
// Layer 3 · lastDayEpisode (01 · Bó) — serbest biçimli gövde
// ─────────────────────────────────────────────
...
// resmi "gap" aparatı bilinçli olarak çıkarıldı — bkz. dosya başı notu.
const bodyEn = [
  block("h2", ["The launch"]),
  block("normal", [
  ...
  block("normal", [
    "Operating under the parent's banking licence removed the licensing timeline and cost — and in exchange, the product inherited RBS's compliance surface, incident process and audit trail. That is a trade with a shape, not a mistake and not something any rule forced.",
  ]),
  note(
  ...
  block("normal", [
    "In the days around launch, App Store reviews dated before the public availability date were spotted by users and press — some later reported to match the names of RBS employees. RBS confirmed that around 2,800 people, including staff, had taken part in a pre-launch trial programme (",
  ...
  block("normal", [{ text: "— RBS spokesperson, as reported by City AM", bold: false }]),
  block("normal", [
  ...
  block("normal", [
    "No rule closed Bó. There was no supervisory action, no enforcement, no licence problem, no insolvency. The PSD2 reissue was a rule doing something concrete and expensive to a young card estate — it was not what ended the product. Holding those two apart is the point of this episode.",
  ]),
  note(
