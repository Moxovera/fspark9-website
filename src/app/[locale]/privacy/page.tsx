...
export async function generateMetadata({
  ...
  const seo = { ...toLegalPageSeo(seoResult), ...legalSeo[locale === "tr" ? "tr" : "en"].privacy };
...
export default async function PrivacyPage({
  ...
  const tab = legal.tabs.find((t) => t.href === "/privacy");
  return (
    <>
      <JsonLd data={breadcrumbJsonLd(loc, [{ name: tab?.label ?? page.hero.title, path: getPathname({ href: "/privacy", locale: loc }) }])} />
      <LegalPageView page={page} legal={legal} current="/privacy" />
    </>
  );
}
