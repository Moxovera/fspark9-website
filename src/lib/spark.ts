 ...
 * ve gelecekler (status coming, format.comingIssues) Sanity'den; gün
 * sayısı launchDate/closureDate'ten hesaplanır, hiç elle yazılmaz.
...
export async function loadFormatIssues(
  ...
  const published: SparkIssue[] = (page?.episodes ?? []).map((episode) => ({
    ...
    statusLabel: episode.publishedAt ? formatShortDate(episode.publishedAt, locale) : hub.launchDateLabel,
  }));
...
/** Formatın diğer dildeki slug'ı (formatlar iki dilde aynı sırada). */
